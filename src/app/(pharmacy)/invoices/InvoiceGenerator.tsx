...
export default function InvoiceGenerator() {
  ...
  const handlePreview = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const res = await fetch(
      `/api/invoices/preview?start=${periodStart}&end=${periodEnd}`
    );
      ...
      {preview && (
        <div className="mt-4 border-t pt-4">
          <p className="text-sm font-medium text-gray-700">
            {preview.orders.length} delivered orders
          </p>
          ...
          <button
            ...
            disabled={loading}
            className="w-full mt-3 bg-blue-600 text-white py-2 rounded-lg text-sm font-medium hover:bg-blue-700 disabled:opacity-50"
          >
